...
export const schlosserApi = {
  ...
  // PRECIFICAÇÃO (HOTFIX)
  // =============================
  // O front chama: schlosserApi.calculatePrice(product, role)
  // No bundle anterior isso estava ausente => "...calculatePrice is not a function" => tela branca.
  //
  // ✅ Hotfix: enquanto não plugamos múltiplas tabelas/colunas de preço,
  // usamos o preço base já carregado do Sheets (coluna AA => product.precoH).
  // Depois a gente evolui para: tabela do cliente + colunas de preço por tabela.
  calculatePrice(product, role = 'public', clientOrTable = null) {
    try {
      const p = product || {};
      // preço base vindo do getProducts(): precoH
      const base = Number(p.precoH || p.preco || p.price || 0) || 0;

      // Se quiser, aqui dá pra diferenciar por role no futuro.
      // Por enquanto, mantemos igual para não quebrar o app.
      return base;
    } catch (e) {
      return 0;
    }
  ...
  // Opcional: transparência de qual "tabela" foi usada (placeholder)
  getEffectiveTable(product, role = 'public', clientOrTable = null) {
    // Hoje todos usam o mesmo preço base (AA)
    return 'AA';
  },
  ...
  async confirmOrder(orderId, token = null) {
    ...
    try {
      const res = await fetch('/api/orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'confirm', orderId, token }) });
      const out = await res.json();
      if (!out?.ok) throw new Error(out?.error || 'Falha ao confirmar pedido');
      return out;
    } catch (e) { console.error('confirmOrder error', e); throw e; }
  },

  async cancelOrder(orderId) {
    if (!orderId) throw new Error('orderId é obrigatório');
    try {
      const res = await fetch('/api/orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'cancel', orderId }) });
      const out = await res.json();
      if (out?.ok) return { success: true, server: true, out };
    } catch (e) { console.warn('cancelOrder: falha ao chamar server - fallback local', e); }
    ...
    } catch (e) { console.error('cancelOrder fallback error', e); return { success: false, error: e.message }; }
  },
  ...
  async getOrders(role = 'public', key = null, filters = {}) {
    try {
      ...
      try {
        ...
        if (res.ok) { const out = await res.json(); if (out?.ok && Array.isArray(out.orders)) return out.orders; }
      } catch (e) {}
